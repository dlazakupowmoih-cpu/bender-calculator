...
const fractionOptions = [
  {value:0,label:"0"},{value:1/16,label:"1/16"},{value:1/8,label:"1/8"},{value:3/16,label:"3/16"},
  {value:1/4,label:"1/4"},{value:5/16,label:"5/16"},{value:3/8,label:"3/8"},{value:7/16,label:"7/16"},
  {value:1/2,label:"1/2"},{value:9/16,label:"9/16"},{value:5/8,label:"5/8"},{value:11/16,label:"11/16"},
  {value:3/4,label:"3/4"},{value:13/16,label:"13/16"},{value:7/8,label:"7/8"},{value:15/16,label:"15/16"}
];
...
function formatInches(inches){
  ...
  let total16=Math.round(inches*16);
  let whole=Math.floor(total16/16);
  let rem=total16%16;
...
// --------- Одиночный гиб: кастомный угол, ограничения ---------
...
numberInput.oninput=()=>{
  ...
  if(val>89) val=89;
  slider.value=val; numberInput.value=val;
};
...
// --------- Функция расчёта одиночного гиба ---------
function calculateSingleBend(){
  ...
  if(type==="stub"&&angleDeg!==90){alert("Для 90° Stub-up угол должен быть ровно 90°."); return;}
  if(type!=="stub"&&angleDeg===90){alert("Угол 90° доступен только для 90° Stub-up.");return;}
  ...
  if(type==="stub"){
    ...
    res+=`3) Аккуратно согните трубу ровно на 90°.\n`;
  } else if(type==="offset"){
    const hyp=val2/sin;
    const adj=hyp*cos;
    ...
    res+=`6) Совместите метку M2 со стрелкой, согните снова на ${angleDeg}°.\n`;
    ...
    res+=`2) Сгибайте по меткам: M2 на двойной угол ${(angleDeg*2).toFixed(1)}°, затем M1 и M3 на угол ${angleDeg}°.\n`;
...
addMultiBend(); // первый изгиб по умолчанию
...
function calculateMultiRun() {
  ...
  for (const row of bendRows) {
    ...
    const angleSelect = row.querySelector(`select.multi-bend-angle[data-id="${id}"]`);
    const angleDeg = angleSelect.value === 'custom' ? parseFloat(row.querySelector(`input.multi-bend-custom-number[data-id="${id}"]`).value) : parseFloat(angleSelect.value);
  ...
  let currentPos = 0;
  let instruction = `Тип трубы: ${ctype.toUpperCase()} ${csize}" (Take-up: ${takeup}", CLR: ${clr}")\n\nMulti-Run изгибы: ${bends.length}\n\n`;
  ...
  bends.forEach((b, idx) => {
    ...
    if (b.bendType === 'single') { // Kick
      const mark = currentPos + b.distance - takeup;
      instruction += ` - Отмерьте от торца трубы ${formatInches(mark)} и сделайте метку\n`;
      instruction += ` - Совместите метку со стрелкой и согните на угол ${b.angleDeg}°\n\n`;
      currentPos += b.distance;
    } else if (b.bendType === 'stub') {
      const angle = (b.angleDeg !== 90) ? 90 : b.angleDeg;
      const mark = currentPos + b.distance - takeup;
      instruction += ` - Отмерьте от торца трубы ${formatInches(mark)} и сделайте метку\n`;
      instruction += ` - Совместите метку со стрелкой и согните ровно на 90°\n\n`;
      currentPos += b.distance;
      ...
      instruction += ` - Совместите метку M1 со стрелкой, согните трубу на ${b.angleDeg}°\n`;
      instruction += ` - Поверните трубу, отметьте и совместите метку M2: ${formatInches(M2)}\n`;
      instruction += ` - Согните снова на ${b.angleDeg}°\n\n`;
      ...
      instruction += ` - Угол: ${b.angleDeg}°\n`;
      instruction += ` - Метки: M1 = ${formatInches(M1)}, M2 = ${formatInches(M2)}, M3 = ${formatInches(M3)}\n`;
      instruction += ` - Сгибайте M2 на двойной угол ${(b.angleDeg * 2).toFixed(1)}°, M1 и M3 на угол ${b.angleDeg}°\n\n`;
      ...
      instruction += ` - Угол: ${b.angleDeg}°\n`;
      instruction += ` - Метки: M1 = ${formatInches(M1)}, M2 = ${formatInches(M2)}, M3 = ${formatInches(M3)}, M4 = ${formatInches(M4)}\n`;
      instruction += ` - Сгибы по меткам в порядке: M2 → M1, M3, M4\n\n`;
...
function addParallelPipe() {
  ...
  row.querySelector(`select.parallel-pipe-type[data-id="${newId}"]`).addEventListener('change', () => populateParallelPipeSizes(newId));

  row.querySelector('.parallel-remove-pipe-btn').addEventListener('click', () => { row.remove(); });
}
...
function calculateParallelRun() {
  ...
  instructions += `Высота подъёма: ${formatInches(rise)}\n`;
  instructions += `Угол изгиба: ${angleDeg}°\n\n`;

  let currentOffset = 0;
  pipes.forEach((pipe, idx) => {
    const { takeup, clr } = pipe.specs;
    instructions += `Труба #${idx + 1} (${pipe.type.toUpperCase()} ${pipe.size}"):\n`;

    if (bendType === 'stub') {
      const markPos = basePoint + currentOffset - takeup;
      instructions += `1) Отмерьте от торца трубы ${formatInches(markPos)} и сделайте метку.\n`;
      instructions += `2) Совместите метку со стрелкой трубогиба.\n`;
      instructions += `3) Согните трубу ровно на 90°.\n\n`;
      ...
      const M2 = M1 + hyp;
      instructions += `1) Отметьте метку M1: ${formatInches(M1)}.\n`;
      instructions += `2) Совместите со стрелкой, согните на угол ${angleDeg}°.\n`;
      instructions += `3) Поверните трубу, отмерьте метку M2: ${formatInches(M2)}.\n`;
      instructions += `4) Совместите со стрелкой, согните снова на угол ${angleDeg}°.\n\n`;
    } else {
      instructions += 'Тип изгиба пока не поддерживается подробно.\n\n';
    }
